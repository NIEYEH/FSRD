...
try:
    from optimum.habana.utils import set_seed as habana_set_seed
except:
    habana_set_seed = None
        ...
        if args.validation_prompts is not None:
            ...
            for i in range(len(args.validation_prompts)):
                try:
                    ...
                    logger.warning(f"Failed to generate validation image {i}: {str(e)}")
                    continue

        for tracker in accelerator.trackers:
            if tracker.name == "tensorboard":
                if args.validation_prompts is not None and len(images) > 0:
...
def parse_args():
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Root directory containing custom dataset with llava_output subdirectories."
        ),
    )
    parser.add_argument(
        "--custom_image_root",
        ...
        default=None,
        help=(
            "Root directory containing image files. If not specified, will use custom_data_root."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=float,
        default=50.0,
        help="approximately percentage of the time that expanded captions is chosen.",
    )
...
def main():
    ...
    # set device (before accelerator initialization, don't use logger yet)
    ...
    if args.device == "hpu":
        ...
        if os.path.exists(os.path.join(checkpoint_path, "unet")):
            ...
            if os.path.exists(os.path.join(parent_dir, "scheduler")):
                noise_scheduler = DDPMScheduler.from_pretrained(parent_dir, subfolder="scheduler")
                logger.info(f"Loading scheduler from parent directory: {parent_dir}")
            else:
                noise_scheduler = DDPMScheduler.from_pretrained("stabilityai/stable-diffusion-2-base", subfolder="scheduler")
                logger.warning(f"Scheduler not found in checkpoint, using default SD2 scheduler")
        else:
            noise_scheduler = DDPMScheduler.from_pretrained(checkpoint_path, subfolder="scheduler")

    checkpoint_path = args.pretrained_model_name_or_path
    if os.path.exists(os.path.join(checkpoint_path, "unet")):
        parent_dir = os.path.dirname(checkpoint_path)
        if os.path.exists(os.path.join(parent_dir, "tokenizer")):
            tokenizer = CLIPTokenizer.from_pretrained(parent_dir, subfolder="tokenizer", revision=args.revision)
            logger.info(f"Loading tokenizer from parent directory: {parent_dir}")
        else:
            tokenizer = CLIPTokenizer.from_pretrained("stabilityai/stable-diffusion-2-base", subfolder="tokenizer")
            logger.warning(f"Tokenizer not found in checkpoint, using default SD2 tokenizer")
    else:
        tokenizer = CLIPTokenizer.from_pretrained(
            checkpoint_path, subfolder="tokenizer", revision=args.revision
        )
    ...
    if not args.train_text_encoder:
        with ContextManagers(deepspeed_zero_init_disabled_context_manager()):
            if os.path.exists(os.path.join(args.pretrained_model_name_or_path, "text_encoder")):
                text_encoder = CLIPTextModel.from_pretrained(
                    args.pretrained_model_name_or_path, subfolder="text_encoder"
                ).to(accelerator.device)
                logger.info(f"Loading text_encoder from checkpoint: {args.pretrained_model_name_or_path}")
            else:
                text_encoder = CLIPTextModel.from_pretrained(
                    args.pretrained_model_name_or_path, subfolder="text_encoder", revision=args.revision
                ).to(accelerator.device)
            
            if os.path.exists(os.path.join(args.pretrained_model_name_or_path, "unet")):
                parent_dir = os.path.dirname(args.pretrained_model_name_or_path)
                if os.path.exists(os.path.join(parent_dir, "vae")):
                    vae = AutoencoderKL.from_pretrained(parent_dir, subfolder="vae")
                    logger.info(f"Loading VAE from parent directory: {parent_dir}")
                else:
                    vae = AutoencoderKL.from_pretrained("stabilityai/stable-diffusion-2-base", subfolder="vae")
                    logger.warning(f"VAE not found in checkpoint, using default SD2 VAE")
            else:
        ...
        if os.path.exists(os.path.join(args.pretrained_model_name_or_path, "text_encoder")):
            text_encoder = text_encoder_cls.from_pretrained(
                args.pretrained_model_name_or_path, subfolder="text_encoder")
            logger.info(f"Loading text_encoder from checkpoint for training")
        else:
            text_encoder = text_encoder_cls.from_pretrained(
                args.pretrained_model_name_or_path, subfolder="text_encoder", revision=args.revision)
    ...
    if os.path.exists(os.path.join(args.pretrained_model_name_or_path, "unet")):
        ...
        logger.info(f"Loading UNet from checkpoint: {args.pretrained_model_name_or_path}")
    else:
        unet = UNet2DConditionModel.from_pretrained(
    ...
    if args.use_ema:
        if os.path.exists(os.path.join(args.pretrained_model_name_or_path, "unet_ema")):
            logger.info(f"Loading EMA UNet from checkpoint: {args.pretrained_model_name_or_path}/unet_ema")
            ema_unet_model = UNet2DConditionModel.from_pretrained(
                args.pretrained_model_name_or_path, subfolder="unet_ema"
            )
    ...
    if args.custom_data_root is not None:
        import glob
        from PIL import Image

        image_root = args.custom_image_root if args.custom_image_root is not None else args.custom_data_root
        
        # Use the custom_data_root parameter to construct data directories
        data_dirs = [
            # Requires manual addition.
            os.path.join(args.custom_data_root, ""),
        ...
        all_data = []
        for data_dir, image_dir in zip(data_dirs, image_dirs):
            ...
            json_files = glob.glob(os.path.join(data_dir, "*_expanded.json"))
            ...
            for json_file in json_files:
                try:
                    with open(json_file, 'r') as f:
                        data = json.load(f)
                        image_id = data.get('image_id', os.path.basename(json_file).replace('_expanded.json', ''))
                        ...
                        if image_path and os.path.exists(image_path):
                            all_data.append({
                                'image': image_path,
                                'coca_caption': data.get('coca_caption', ''),
                                'spatial_caption': data.get('spatial_caption', ''),
                                'original_caption': data.get('original_caption', ''),
                                'second_optimize_caption': data.get('second_optimize_caption', ''),
                                'image_id': image_id
                            })
                        else:
                            if len(all_data) < 10:  
                                logger.warning(f"Image not found for {json_file}: {image_id}")
                except Exception as e:
                    if len(all_data) < 10: 
                        logger.warning(f"Error loading {json_file}: {str(e)}")
        
        logger.info(f"Loaded {len(all_data)} samples from custom dataset")
        
        if len(all_data) == 0:
            raise ValueError(f"No valid samples found in {args.custom_data_root}")
        
        from datasets import Dataset
        dataset = Dataset.from_list(all_data)
        
        if args.dataset_size is not None:
            dataset_size = int(args.dataset_size * len(dataset)) if args.dataset_size < 1 else int(args.dataset_size)
            dataset = dataset.shuffle(seed=args.seed).select(range(min(dataset_size, len(dataset))))
            logger.info(f"Dataset truncated to {len(dataset)} samples")
        ...
        if args.val_split > 0:
            ...
            for item in train_data:
                expanded_data_list.append({
                    **item,
                    '_preferred_caption': 'second_optimize_caption'
                })
                if rng.random() < 0.5:
                    expanded_data_list.append({
                        **item,
                        '_preferred_caption': 'spatial_caption'
                    })
                else:
                    caption_key = 'original_caption' if 'sprightcoco' in item.get('image', '') else 'coca_caption'
                    expanded_data_list.append({
                        **item,
                        '_preferred_caption': caption_key
                    })
            
            from datasets import Dataset
            expanded_train = Dataset.from_list(expanded_data_list)
            ...
            logger.info(f"Dataset expanded from {len(train_data)} to {len(expanded_train)} samples")
            logger.info(f"Train: {len(dataset['train'])}, Validation: {len(dataset['validation'])}")
        else:
            dataset = datasets.DatasetDict({'train': dataset})
            logger.info(f"Train: {len(dataset['train'])}, No validation split")
    ...
    def tokenize_captions(examples, is_train=True):
        ...
        is_batched = isinstance(examples.get('image', None), list)
        ...
        if is_batched:
            ...
            for idx in range(batch_size):
                sample = {k: v[idx] if isinstance(v, list) else v for k, v in examples.items()}
                
                if '_preferred_caption' in sample and sample['_preferred_caption']:
                    caption_column = sample['_preferred_caption']
                else:
                    column_lists = ['second_optimize_caption', 'spatial_caption', args.general_caption]
                    weights = [args.expanded_percent, args.spatial_percent, 100 - args.expanded_percent - args.spatial_percent]
                    caption_column = random.choices(column_lists, weights=weights, k=1)[0]

                if caption_column == 'second_optimize_caption' and not sample.get('second_optimize_caption'):
                    caption_column = 'spatial_caption'
                
                if caption_column == 'spatial_caption' and not sample.get('spatial_caption'):
                    caption_column = args.general_caption
                
                if caption_column == 'coca_caption' and 'sprightcoco' in sample.get('image', ''):
                    if sample.get('original_caption'):
                        caption_column = 'original_caption'
                    
                if args.general_caption == "original_caption" and caption_column == args.general_caption:
                    if not sample.get(caption_column):
                        caption_column = "coca_caption"
                elif args.spatial_caption_type == "short" and caption_column == spatial_caption:
                    if sample.get(caption_column):
                        sample['short_spatial_caption'] = get_random_sentence(sample[caption_column])
            ...
            if '_preferred_caption' in examples and examples['_preferred_caption']:
                caption_column = examples['_preferred_caption']
            else:
                column_lists = ['second_optimize_caption', 'spatial_caption', args.general_caption]
                weights = [args.expanded_percent, args.spatial_percent, 100 - args.expanded_percent - args.spatial_percent]
                caption_column = random.choices(column_lists, weights=weights, k=1)[0]
            ...
            if caption_column == 'second_optimize_caption' and caption_column not in examples.keys():
                caption_column = 'spatial_caption'
            
            if caption_column == 'spatial_caption' and caption_column not in examples.keys():
                caption_column = args.general_caption

            if caption_column == 'coca_caption' and 'sprightcoco' in examples.get('image', ''):
                if 'original_caption' in examples.keys() and examples['original_caption']:
                    caption_column = 'original_caption'
            ...
            captions = []
            caption_value = examples.get(caption_column, '')
            if not isinstance(caption_value, list):
                caption_value = [caption_value]

            for caption in caption_value:
    ...
    def preprocess_train(examples):
        ...
        if isinstance(examples["image"][0] if isinstance(examples["image"], list) else examples["image"], str):
            images = [Image.open(img_path).convert("RGB") for img_path in (examples["image"] if isinstance(examples["image"], list) else [examples["image"]])]
        else:
    ...
    if accelerator.is_main_process:
        ...
        logger.info(f"  Learning Rate (UNet): {args.learning_rate}")
        logger.info(f"  Learning Rate (Text Encoder): {args.text_encoder_lr}")
        ...
        logger.info(f"  Caption Distribution:")
        logger.info(f"    - Expanded: {args.expanded_percent}%")
        logger.info(f"    - Spatial: {args.spatial_percent}%")
        logger.info(f"    - General: {100 - args.expanded_percent - args.spatial_percent}%")
        logger.info("=" * 80)
    ...
    if args.resume_from_checkpoint:
        ...
        if args.resume_from_checkpoint != "latest":
            if os.path.isdir(args.resume_from_checkpoint):
                ...
                logger.error(f"Checkpoint path does not exist: {args.resume_from_checkpoint}")
            ...
            if checkpoint_dirs:
                ...
                checkpoint_path = checkpoint_dirs[0][1]
                logger.info(f"Found {len(checkpoint_dirs)} checkpoints, using latest: {checkpoint_path}")
            else:
                logger.warning(
        ...
        if args.resume_from_checkpoint and checkpoint_path:
            try:
                ...
                logger.info(f"  Remaining steps: {args.max_train_steps - global_step}")
    ...
    if accelerator.is_main_process:
        ...
        logger.info(f"  Starting from epoch: {first_epoch}")
        logger.info(f"  Starting from step: {initial_global_step}")
        logger.info(f"  Target steps: {args.max_train_steps}")
        logger.info(f"  Steps to train: {args.max_train_steps - initial_global_step}")
        logger.info("=" * 80)
    ...
    pipeline_model_path = args.pretrained_model_name_or_path
    if os.path.exists(os.path.join(args.pretrained_model_name_or_path, "unet")):
        parent_dir = os.path.dirname(args.pretrained_model_name_or_path)
        if os.path.exists(os.path.join(parent_dir, "model_index.json")):
            pipeline_model_path = parent_dir
            logger.info(f"Using parent directory for pipeline creation: {pipeline_model_path}")
        else:
            pipeline_model_path = "stabilityai/stable-diffusion-2-base"
            logger.warning(f"model_index.json not found in parent directory, using SD2 base for pipeline")
    ...
    if args.device == "hpu":
        pipeline = GaudiStableDiffusionPipeline.from_pretrained(
            ...
            revision=args.revision if pipeline_model_path != "stabilityai/stable-diffusion-2-base" else None,
    ...
    pipeline.save_pretrained(os.path.join(args.output_dir, "initial_model"))
    logger.info(f"Initial model saved to {os.path.join(args.output_dir, 'initial_model')}")
    
    text_train_active = False

    max_epochs = args.num_train_epochs + 2 
    for epoch in range(first_epoch, max_epochs):
        ...
        if args.train_text_encoder and global_step > args.freeze_text_encoder_steps:
            text_encoder.train()
        train_loss = 0.0
        logger.info(f"Starting epoch {epoch}/{args.num_train_epochs} (global_step: {global_step}/{args.max_train_steps})")
        ...
        logger.info(f"Epoch {epoch} completed. Global step: {global_step}/{args.max_train_steps}")
        ...
        if accelerator.is_main_process and args.device == "hpu":
            if args.device == "hpu":
    ...
    logger.info("Training completed! Running final validation...")
    ...
    if accelerator.is_main_process:
        ...
        final_pipeline_path = pipeline_model_path  
        ...
        if args.device == "hpu":
            pipeline = GaudiStableDiffusionPipeline.from_pretrained(
                ...
                vae=vae,
                unet=unet,
                revision=args.revision if final_pipeline_path != "stabilityai/stable-diffusion-2-base" else None,
                scheduler=noise_scheduler,
            )
        else:
            pipeline = StableDiffusionPipeline.from_pretrained(
            ...
            try:
                ...
                for i in range(len(args.validation_prompts)):
                    try:
                        ...
                        logger.warning(f"Failed to generate image for prompt {i}: {str(e)}")
                        continue
                
                logger.info(f"Successfully generated {len(images)} validation images")
            except Exception as e:
                logger.error(f"Final validation failed: {str(e)}")
                logger.info("Continuing with model saving...")
